...
from torch_geometric.nn import APPNP as APPNP_
...
cfg.add_argument("--hidden-dim", type=int, default=64)
...
class APPNP(RecSysArch):
    ...
    def __init__(
        ...
        self.fcs = nn.Sequential(
            ...
            nn.Linear(cfg.embedding_dim, cfg.hidden_dim, bias=False),
            nn.ReLU(),
            nn.Dropout(p=cfg.dropout_rate),
            nn.Linear(cfg.hidden_dim, cfg.embedding_dim, bias=False),
        )
        self.convs = APPNP_(
            K=num_layers, alpha=cfg.alpha, normalize=False
        )
    ...
    def forward(
        ...
        features = self.convs(features, self.graph.adj_t)
