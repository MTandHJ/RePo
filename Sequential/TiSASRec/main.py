...
cfg.add_argument('--l2_emb', default=0.00005, type=float)
...
class TimeAwareMultiHeadAttention(torch.nn.Module):
    ...
    def __init__(self, embed_dim: int, head_num: int, dropout_rate: float):
        ...
        self.head_size = embed_dim // head_num
    ...
    def forward(
        ...
        padding_mask = padding_mask.repeat(self.head_num, 1, 1)
...
class TimeMapper(RowMapper):
    ...
    def _time2matrix(self, times: Iterable) -> Iterable:
        times = np.array(times, dtype=np.int).reshape((-1, 1))
        min_time = np.unique(times)
        min_time = max(1, np.min(min_time[1:] - min_time[:-1]).item())
        matrix = np.abs(times - times.T) / min_time
